    ...
    <div className="card flex-shrink-0 w-full  shadow-2xl bg-base-100 py-8">
      <form onSubmit={handleSubmit} className="card-body">
        ...
        <div className="form-control">
          ...
          <input type="email" onBlur={(e)=>setUserEmail(e.target.value)} name='email' placeholder="email" className="input input-bordered text-xl" required />
        </div>
        <div className="form-control">
          <label className="label">
          ...
          <label className="label">
            <p onClick={passwordReset}  className="label-text-alt link text-lg link-hover">Forgot password?</p>
          </label>
        </div>
        <div className="form-control mt-6">
          <button type='submit'  className="btn hover:bg-blue-500 bg-blue-600 outline-0 border-0 text-white">Login</button>
        </div>
      </form>
      ...
      <p onClick={handleGithubSign} className='h-12 w-12 bg-slate-300 rounded-full text-black flex justify-center items-center'><FaGithub></FaGithub></p>
        ...
        <p className="py-4 text-center label-text-alt link link-hover font-medium text-base">New To Genius Car Please? <Link to='/signup' className='text-orange-600'>Sign Up</Link></p>
    </div>
  </div>
</div>
    );
